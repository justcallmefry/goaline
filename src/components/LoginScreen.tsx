...
// FIX: Import from the SSR package, not the base package
import { createBrowserClient } from '@supabase/ssr';

export function LoginScreen() {
  // FIX: Use createBrowserClient so cookies are handled correctly for the server
       ...
       <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-500">
          <div className="p-8 text-center">
             ...
             <button 
                ...
                className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:border-indigo-200 hover:bg-slate-50 text-slate-700 font-bold py-4 rounded-xl transition-all shadow-sm group cursor-pointer relative"
             >
                {isRedirecting ? (
