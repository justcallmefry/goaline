...
export function AdminToolModal({ isOpen, onClose, onSave, initialData }: AdminToolModalProps) {
  const [formData, setFormData] = useState({
    name: '', description: '', affiliate_link: '', category: 'General', pricing: '', value_prop: '', logo_url: ''
  });
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (initialData) {
      // FIX: Use || '' to ensure no value is ever undefined/null (causes the console error)
      setFormData({
        ...
        value_prop: initialData.value_prop || '',
        logo_url: initialData.logo_url || ''
      });
    } else {
      setFormData({ name: '', description: '', affiliate_link: '', category: 'General', pricing: '', value_prop: '', logo_url: '' });
    }
  }, [initialData, isOpen]);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
                ...
                {/* NEW: Logo URL Field */}
                <div className="col-span-2">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Logo Image URL</label>
                    <input type="url" placeholder="https://example.com/logo.png" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none" value={formData.logo_url} onChange={(e) => setFormData({...formData, logo_url: e.target.value})} />
                </div>
                <div className="col-span-2">
